...
char **argv;
{
     ...
     double phi0,r0;
     double beta; // declaring the temperature gradient
     double dtheta;
     double logscale;
     int ntheta;
     FILE *fp,*gp;
	 ...
	 /* sscanf(*++argv,"%s",infile);*/  /* input file name */
         ...
         beta = -0.0065; // temp gradient in Km^-1
         r0=6365.5; 
     ...
     rray[0]=r0;
     phi[0]=phi0;
     n[0]=1.0+(77.6*1013.25)/288.15/1e6;
     phib[0]=pi/2.0-phi0;
     ...
     for (i=1;i<=ntheta;i++)
        {
           ...
           T[i] = (beta * (rray[i] - rray[0])) + 288.15;
           init_P[i] = (((beta * (rray[i] - rray[0])) + 288.15)) / 288.15; // (in hPa) ERROR WITH DIVISOR HERE???
           double exp_P =  (-9.80665 * 0.028964420) / (beta * 8.31432);
           // printf("%f", exp_P);
           P[i] = 1013.25 * pow(init_P[i], exp_P);
          printf("%lf", init_P[i]);
          //  printf("%f", T[i]);

          //  n[i]=1.0+330.0/exp((rray[i]-rray[0])/8.0)/1e6; /* REPLACE THIS WITH SOMETHING BETTER */
           n[i] = 1 + (77.6 * P[i]/T[i])/1e6;
