...
def chi_squared(funct, x, y, x_errors, y_errors, w_initial):
    ...
    plt.errorbar(x, y, y_errors, x_errors, fmt = "r.", capsize = 3, elinewidth = 0.5, markeredgewidth = 0.5, LineStyle = "none", Label = "Data Points")
    # the last argument for the two lines below, is for the legend
    plt.plot(x, y, Marker = ".", MarkerSize = 0.2, MarkerEdgeColor = "k", MarkerFaceColor = "k", markeredgewidth = 0.5, LineStyle = "none")
    ...
    plt.xlabel(" $\\sqrt{h}$ (cm)$^\\frac{1}{2}$", fontsize = 12)
    
    plt.ylabel("Wave Speed $v$ (cms$^{-1}$)", fontsize = 12)
    plt.legend(loc = "lower right", title = "Legend", fontsize = 10)
    plt.axis([0.4, 1.1, 5, 27.5])
    # plt.xlim(0, 6)
    ...
    plt.savefig("Water_Wave_Depths.pdf")