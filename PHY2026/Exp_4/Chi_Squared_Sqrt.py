...
def chi_squared(funct, x, y, y_errors, w_initial):
    ...
    # print("{:f}" .format(u[2]))
    print("The y errors are: ", y_errors)

    def misfit_gvals(x_results):
        g_results = []
        for x_result in x_results:
            g = (u[0] * x_result) + (u[1] * np.sqrt(x_result))
            g_results.append(g)
        return g_results
    ...
    plt.errorbar(x, y, y_errors, fmt = "r.", capsize = 4, elinewidth = 0.1, markeredgewidth = 0.3, markerfacecolor = "none", markersize = 7, LineStyle = "none", Label = "Data Points")
    ...
    plt.plot(x, g_vals, LineWidth = 0.9, LineStyle = "-", Color = "b", label = "Data Model") #, label = "Chi Squared Fit") # 'chi squared line of best fit'
    ...
    plt.savefig("Chi_Squared_Log.pdf")