...
def least_squares(funct, x, y, x_errors, y_errors, w_initial):
    ...
    w = ls_arr
    print("The first coefficient of the least squares fit is:", end = " ")
    print("{:f}" .format(w[0]))
    print("The second coefficient of the least squares fit is:", end = " ")
    print("{:f}" .format(w[1]))
    print("The third coefficient of the least squares fit is:", end = " ")
    print("{:f}" .format(w[2]))

    def misfit_fvals(x_results):
        f_results = []
        for x_result in x_results:
            f = w[0] * (x_result ** 2) + w[1] * x_result + w[2]
            f_results.append(f)
        return f_results
    ...
    plt.savefig("Chi_Squared_Quadratic.pdf")